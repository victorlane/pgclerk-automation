...
variable "ssh_public_key" {
  type        = string
  description = "OpenSSH public key Ansible uses for first contact. Injected by pgclerk at dispatch."
  default     = ""
}
...
# Reserved-for-symmetry — embedded etcd doesn't use these but we
# accept them so the cluster wizard can pass the full jitPlan.
variable "etcd_count" {
  type    = number
  default = 0
}
